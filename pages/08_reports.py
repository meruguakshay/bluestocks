...
if df_companies.empty:
    ...
    if selected_option == "":
        ...
        if df_docs.empty:
            ...
            st.markdown("We verify the availability of these BSE annual report PDF links dynamically.")
            ...
            for idx, row in df_docs.iterrows():
                ...
                if is_available:
                    ...
                    col_status.markdown("🔴 <span style='color: #EF4444; font-weight: bold; background-color: #FEE2E2; padding: 2px 6px; border-radius: 4px;'>Report unavailable</span>", unsafe_allow_html=True)
                    # Show disabled visual or text
                    col_btn.markdown(f"~~Download PDF Link~~ (URL Broken)")
                
                st.markdown("<hr style='margin: 8px 0; border: 0.5px solid #E5E7EB;' />", unsafe_allow_html=True)
