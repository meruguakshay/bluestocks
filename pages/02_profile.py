...
from utils.db import get_companies, get_ratios, get_pl, get_bs, get_cf, get_connection
    ...
    if selected_option == "":
        ...
        if company_row.empty:
            ...
            with card_col:
                ...
                st.markdown(f"**Sector**: {company_row['broad_sector']} | **Sub-Sector**: {company_row['sub_sector']}")
                st.markdown(f"**About**: {company_row['about_company'] if pd.notna(company_row['about_company']) else 'N/A'}")
            ...
            with logo_col:
                ...
                if pd.notna(company_row["company_logo"]) and company_row["company_logo"] != "":
                    st.image(company_row["company_logo"], width=150, fallback="https://via.placeholder.com/150?text=No+Logo")
                else:
                    st.markdown("""
            ...
            if df_ratios.empty or df_pl.empty or df_bs.empty:
                st.warning("Complete financial statements are not available for this company.")
            else:
                # Find latest year
                ...
                if pd.isna(latest_fcf):
                    ...
                    if not latest_cf_row.empty:
                        latest_fcf = latest_cf_row.iloc[0].get("operating_activity", 0) + latest_cf_row.iloc[0].get("investing_activity", 0)
                ...
                if not latest_pl_row.empty and not latest_bs_row.empty:
                    ...
                    equity = latest_bs_row.iloc[0].get("equity_capital", 0) + latest_bs_row.iloc[0].get("reserves", 0)
                ...
                kpi_col1.metric(f"ROE ({latest_year})", fmt_p(latest_ratio.get("return_on_equity_pct")))
                kpi_col2.metric(f"ROCE ({latest_year})", fmt_p(latest_roce))
                kpi_col3.metric(f"Net Profit Margin ({latest_year})", fmt_p(latest_ratio.get("net_profit_margin_pct")))
                kpi_col4.metric(f"D/E Ratio ({latest_year})", fmt_r(latest_ratio.get("debt_to_equity")))
                kpi_col5.metric("Revenue CAGR (5yr)", fmt_p(latest_ratio.get("revenue_cagr_5yr")))
                ...
                if available_years < 10:
                    st.info(f"ℹ️ Partial financial data available: Showing last {available_years} years.")
                ...
                with chart_ro_col:
                    ...
                    for idx, row in df_ratios_10yr.iterrows():
                        ...
                        if not pl_row.empty and not bs_row.empty:
                            ...
                            equity = bs_row.iloc[0].get("equity_capital", 0) + bs_row.iloc[0].get("reserves", 0)
                ...
                conn = get_connection()
                df_pc = pd.read_sql(f"SELECT * FROM prosandcons WHERE company_id = '{ticker}'", conn)
                conn.close()
                
                if df_pc.empty:
                    st.info("No specific pros & cons listed in the database for this company.")
                else:
                    pros_text = df_pc.iloc[0].get("pros")
                    cons_text = df_pc.iloc[0].get("cons")
                    
                    pros_list = [p.strip() for p in pros_text.split("\n") if p.strip()] if pd.notna(pros_text) else []
                    cons_list = [c.strip() for c in cons_text.split("\n") if c.strip()] if pd.notna(cons_text) else []
