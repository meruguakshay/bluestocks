...
def set_preset(roe=0.0, de=10.0, fcf=-2000.0, rev_cagr=-50.0, pat_cagr=-100.0, opm=-50.0, pe=500.0, pb=100.0, div_yield=0.0, icr=0.0):
...
fcf_min = st.sidebar.slider("Min FCF (₹ Cr)", -1000.0, 20000.0, key="scr_fcf", step=50.0)
rev_cagr_min = st.sidebar.slider("Min 5yr Revenue CAGR (%)", -30.0, 100.0, key="scr_rev_cagr", step=1.0)
pat_cagr_min = st.sidebar.slider("Min 5yr PAT CAGR (%)", -50.0, 150.0, key="scr_pat_cagr", step=1.0)
...
div_yield_min = st.sidebar.slider("Min Dividend Yield (%)", 0.0, 15.0, key="scr_div_yield", step=0.2)
icr_min = st.sidebar.slider("Min Interest Coverage (ICR)", 0.0, 50.0, key="scr_icr", step=1.0)
...
if df_ratios.empty or df_companies.empty:
    ...
    df_screener = pd.merge(df_screener, df_companies[["company_id", "company_name", "broad_sector"]], on="company_id", how="inner")
    ...
    df_filtered = df_filtered[df_filtered["return_on_equity_pct"].fillna(-999.0) >= roe_min]
    df_filtered = df_filtered[df_filtered["debt_to_equity"].fillna(999.0) <= de_max]
    df_filtered = df_filtered[df_filtered["free_cash_flow_cr"].fillna(-99999.0) >= fcf_min]
    df_filtered = df_filtered[df_filtered["revenue_cagr_5yr"].fillna(-999.0) >= rev_cagr_min]
    df_filtered = df_filtered[df_filtered["pat_cagr_5yr"].fillna(-999.0) >= pat_cagr_min]
    df_filtered = df_filtered[df_filtered["operating_profit_margin_pct"].fillna(-999.0) >= opm_min]
    ...
    df_filtered = df_filtered[df_filtered["dividend_yield_pct"].fillna(-999.0) >= div_yield_min]
    df_filtered = df_filtered[df_filtered["interest_coverage"].fillna(-999.0) >= icr_min]
    ...
    df_display.columns = [
        "Ticker", "Company Name", "Sector", "Quality Score",
        "ROE (%)", "D/E", "FCF (Cr)",
        "Rev CAGR 5yr (%)", "PAT CAGR 5yr (%)", "OPM (%)",
        "P/E", "P/B", "Div Yield (%)", "ICR"
    ]
