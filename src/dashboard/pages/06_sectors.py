...
from utils.db import (
    get_all_market_cap_for_year,
    get_all_ratios_for_year,
    get_companies,
    get_connection,
)
...
if df_companies.empty:
    ...
    selected_sector = st.selectbox(
        "Select Broad Sector", options=["All Sectors"] + broad_sectors, index=0
    )
    ...
    if df_all.empty:
        ...
        if df_filtered.empty:
            st.warning(
                f"No companies found for sector '{selected_sector}' in {selected_year}."
            )
        else:
            # ────────────────────────────────────────────────────────
            # Bubble Chart
            ...
            st.markdown(
                "*Bubble size represents Market Cap (in ₹ Crores), colored by sub-sector.*"
            )
            ...
            df_plot = df_filtered[
                (df_filtered["market_cap_crore"] > 0) & (df_filtered["sales"] > 0)
            ].copy()
            ...
            df_medians = (
                df_filtered.groupby("sub_sector")[
            ...
            if df_medians.empty:
                ...
                df_medians_sorted = df_medians.dropna(subset=[db_col]).sort_values(
                    by=db_col, ascending=False
                )
                ...
                fig_bar = px.bar(
                    ...
                    y=db_col,
                    text=db_col,
                    labels={"sub_sector": "Sub-Sector", db_col: label_name},
