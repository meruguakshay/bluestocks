...
if df_ratios.empty or df_companies.empty:
    ...
    median_pe = (
        df_merged["pe_ratio"].median()
        if not df_merged.empty
        else df_mcap["pe_ratio"].median()
    )
    ...
    with table_col:
        ...
        df_top = df_top.sort_values(by="composite_quality_score", ascending=False).head(
            5
        )
        ...
        if not df_top.empty:
            ...
            df_top_display["Quality Score"] = df_top_display["Quality Score"].map(
                lambda x: f"{x:.1f}" if pd.notna(x) else "N/A"
            )
            df_top_display["ROE (%)"] = df_top_display["ROE (%)"].map(
                lambda x: f"{x:.2f}%" if pd.notna(x) else "N/A"
            )
            df_top_display["D/E Ratio"] = df_top_display["D/E Ratio"].map(
                lambda x: f"{x:.2f}" if pd.notna(x) else "N/A"
            )
