...
from utils.db import get_connection, get_peers, get_ratios, get_pl, get_bs, get_cf
...
if df_group.empty:
    ...
    for _, p_row in df_group.iterrows():
        ...
        if not df_p_pl.empty and not df_p_bs.empty:
            ...
            equity = (bs_row.get("equity_capital", 0) or 0) + (bs_row.get("reserves", 0) or 0)
    ...
    if target_row.empty:
        ...
        for name, col in metrics:
            ...
            if pd.notna(c_val) and pd.notna(a_val) and pd.notna(max_val) and pd.notna(min_val) and max_val > min_val:
        ...
        hover_company = [f"{lbl}: {fmt_raw(raw, lbl)}" for lbl, raw in zip(labels, raw_company_values)]
        hover_company_closed = hover_company + [hover_company[0]]
        
        hover_average = [f"{lbl}: {fmt_raw(raw, lbl)} (Group Avg)" for lbl, raw in zip(labels, raw_average_values)]
    ...
    df_peer_display = df_peers_metrics[[
        "company_id", "company_name", "roe", "roce", "npm", "de", "fcf", "pat_cagr", "rev_cagr", "eps_cagr"
    ]].copy()
    
    df_peer_display.columns = [
        "Ticker", "Company Name", "ROE (%)", "ROCE (%)", "NPM (%)", "D/E Ratio", "FCF (Cr)", "PAT CAGR (%)", "Rev CAGR (%)", "EPS CAGR (%)"
    ]
    ...
    def highlight_selected(row, selected_ticker):
        if row["Ticker"] == selected_ticker:
            return ['background-color: #D1FAE5; font-weight: bold; color: #111827;'] * len(row)
        return [''] * len(row)
        
    styled_df = df_peer_display.style.apply(highlight_selected, axis=1, selected_ticker=ticker)
