...
from utils.db import get_all_market_cap_for_year, get_companies, get_connection
...
if df_cf.empty or df_companies.empty:
    ...
    df_merged["market_cap_crore"] = (
        df_merged["market_cap_crore"].fillna(100.0).clip(lower=1.0)
    )
    ...
    # ────────────────────────────────────────────────────────
    st.subheader(
        f"Treemap: Nifty 100 Companies by Capital Allocation Pattern ({selected_year})"
    )
    st.markdown(
        "*Box sizes represent Market Cap. Hover/click to inspect patterns and companies.*"
    )
    ...
    selected_pattern = st.selectbox(
        "Select a Pattern to View Companies", options=unique_patterns
    )
    ...
    st.markdown(
        f"**{len(df_filtered)} companies** classified as **{selected_pattern}**:"
    )
