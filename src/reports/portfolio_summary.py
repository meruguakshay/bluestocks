...
def main():
    ...
    df_companies = pd.read_sql("SELECT company_id, company_name, sector_id FROM companies ORDER BY company_id", conn)
    df_sectors = pd.read_sql("SELECT sector_id, broad_sector FROM sectors", conn)
    
    # Load financial tables
    df_pnl = pd.read_sql("SELECT company_id, year, sales, net_profit, operating_profit, depreciation FROM profitandloss", conn)
    df_bs = pd.read_sql("SELECT company_id, year, equity_capital, reserves, borrowings FROM balancesheet", conn)
    df_ratio = pd.read_sql("SELECT company_id, year, return_on_equity_pct, debt_to_equity FROM financial_ratios", conn)
    df_mcap = pd.read_sql("SELECT company_id, year, market_cap_crore FROM market_cap", conn)
    ...
    # Custom styles
    style_header = ParagraphStyle('PortHeader', parent=styles['Heading1'], fontName='Helvetica-Bold', fontSize=14, textColor=colors.white)
    style_subheader = ParagraphStyle('PortSubheader', fontName='Helvetica', fontSize=9, textColor=colors.white, alignment=TA_RIGHT)
    style_sec_title = ParagraphStyle('PortSecTitle', fontName='Helvetica-Bold', fontSize=11, textColor=HexColor('#002B49'), spaceAfter=8)
    
    style_cell_lbl = ParagraphStyle('CellLbl', fontName='Helvetica-Bold', fontSize=8.5, leading=10, textColor=HexColor('#374151'))
    style_cell_val = ParagraphStyle('CellVal', fontName='Helvetica', fontSize=8, leading=10, textColor=HexColor('#1F2937'))
    style_cell_val_bold = ParagraphStyle('CellValBold', fontName='Helvetica-Bold', fontSize=8.5, leading=10, textColor=HexColor('#111827'))
    ...
    for idx, ticker in enumerate(tickers):
        ...
        union_years = sorted(list(set(c_pnl["year"]).intersection(c_bs["year"]).intersection(c_ratio["year"])))
        ...
        if len(union_years) >= 1:
            ...
            # Fetch latest
            lat_pnl = c_pnl[c_pnl["year"] == latest_year].iloc[0] if not c_pnl[c_pnl["year"] == latest_year].empty else {}
            lat_bs = c_bs[c_bs["year"] == latest_year].iloc[0] if not c_bs[c_bs["year"] == latest_year].empty else {}
            lat_ratio = c_ratio[c_ratio["year"] == latest_year].iloc[0] if not c_ratio[c_ratio["year"] == latest_year].empty else {}
            lat_mcap = c_mcap[c_mcap["year"] == latest_year].iloc[0] if not c_mcap[c_mcap["year"] == latest_year].empty else {}
        ...
        if len(union_years) >= 2:
            ...
            # Fetch previous
            p_pnl = c_pnl[c_pnl["year"] == prev_year].iloc[0] if not c_pnl[c_pnl["year"] == prev_year].empty else {}
            p_bs = c_bs[c_bs["year"] == prev_year].iloc[0] if not c_bs[c_bs["year"] == prev_year].empty else {}
            p_ratio = c_ratio[c_ratio["year"] == prev_year].iloc[0] if not c_ratio[c_ratio["year"] == prev_year].empty else {}
            p_mcap = c_mcap[c_mcap["year"] == prev_year].iloc[0] if not c_mcap[c_mcap["year"] == prev_year].empty else {}
        ...
        header_data = [
            [Paragraph(f"{comp_name} ({ticker})", style_header), Paragraph(f"Sector: {sector_name}", style_subheader)]
        ]
        ...
        story.append(Paragraph(f"YoY Performance Dashboard (Comparing {prev_year} vs {latest_year})", style_sec_title))
        ...
        metrics_table = Table(rows, colWidths=[2.5*inch, 1.2*inch, 1.2*inch, 1.2*inch, 1.2*inch])
    ...
    # Build document
    doc.build(story)
    print(f"Successfully generated portfolio summary PDF: {dest_path} ({os.path.getsize(dest_path)} bytes)")
