...
def main():
        ...
        try:
            ...
            if success:
                ...
                if generated_count % 10 == 0 or idx == len(tickers) - 1:
                    print(
                        f"  [{idx+1}/{len(tickers)}] Generated tearsheet for {ticker}..."
                    )
            else:
                skipped_list.append(
                    {
            ...
            skipped_list.append(
                {"company_id": ticker, "reason": f"Exception: {e!s}"}
            )
    ...
    print(
        f"Skipped {len(skipped_list)} companies. Skipped list saved to {SKIPPED_FILE}."
    )
    ...
    for idx, row in df_sectors.iterrows():
        ...
        try:
            ...
            if success:
                print(
                    f"  Sector [{idx+1}/{len(df_sectors)}] Generated report for {sector_name} -> {dest_path}"
                )
            else:
                print(
                    f"  Sector [{idx+1}/{len(df_sectors)}] Skipped empty sector: {sector_name}"
                )
        except Exception as e:
