...
def generate_revenue_profit_chart(ticker, df_pnl, dest_path):
    ...
    ax.set_title(f'10-Year Revenue & Net Profit Trend', fontsize=9, fontweight='bold', color='#002B49')
    ax.set_xticks(x)
...
def generate_roe_roce_chart(ticker, df_pnl, df_bs, df_ratios, dest_path):
    ...
    union_years = sorted(list(set(df_pnl["year"]).intersection(df_bs["year"]).intersection(df_ratios["year"])))
...
def generate_balancesheet_composition_chart(ticker, df_bs, dest_path):
    ...
    years = [y.split("-")[0] for y in df_last_10["year"]]
    equity = [to_float(r.get("equity_capital")) + to_float(r.get("reserves")) for _, r in df_last_10.iterrows()]
    borrowings = [to_float(r.get("borrowings")) for _, r in df_last_10.iterrows()]
    other_liab = [to_float(r.get("other_liabilities")) for _, r in df_last_10.iterrows()]
    ...
    ax.bar(years, other_liab, bottom=bottom_3, label='Other Liabilities', color='#D1D5DB')
    
    ax.set_ylabel('Amount (Cr)', fontsize=8, color='#002B49', fontweight='bold')
    ax.set_title('Balance Sheet Composition', fontsize=9, fontweight='bold', color='#002B49')
...
def generate_cashflow_waterfall_chart(ticker, df_cf, dest_path):
    ...
    for i in range(3):
        ax.plot([i, i+1], [bottoms[i] + heights[i], bottoms[i] + heights[i]], color='gray', linestyle=':', alpha=0.8)
        
    ax.set_ylabel('Amount (Cr)', fontsize=8, color='#002B49', fontweight='bold')
    ax.set_title(f'Latest Year Cash Flow Waterfall ({latest_row.get("year")})', fontsize=9, fontweight='bold', color='#002B49')
    ax.grid(axis='y', linestyle='--', alpha=0.5)
...
def generate_tearsheet(ticker, dest_path):
    ...
    company = pd.read_sql(f"SELECT * FROM companies WHERE company_id = '{ticker}'", conn)
    if company.empty:
        conn.close()
        raise ValueError(f"Company {ticker} not found in database.")
    c_info = company.iloc[0]
    
    pnl = pd.read_sql(f"SELECT * FROM profitandloss WHERE company_id = '{ticker}' ORDER BY year", conn)
    bs = pd.read_sql(f"SELECT * FROM balancesheet WHERE company_id = '{ticker}' ORDER BY year", conn)
    cf = pd.read_sql(f"SELECT * FROM cashflow WHERE company_id = '{ticker}' ORDER BY year", conn)
    ratios = pd.read_sql(f"SELECT * FROM financial_ratios WHERE company_id = '{ticker}' ORDER BY year", conn)
    mcap = pd.read_sql(f"SELECT * FROM market_cap WHERE company_id = '{ticker}' ORDER BY year", conn)
    
    # Load sectors
    sector_id = c_info.get("sector_id")
    sector_row = pd.read_sql(f"SELECT broad_sector FROM sectors WHERE sector_id = {sector_id}", conn)
    sector_name = sector_row.iloc[0]["broad_sector"] if not sector_row.empty else "Unknown"
    
    conn.close()
    
    # Skip logic: Tick check for < 3 conformed years
    conformed_years = set(pnl["year"]).intersection(bs["year"]).intersection(cf["year"])
    if len(conformed_years) < 3:
        return False
        
    sorted_years = sorted(list(conformed_years))
    latest_year = sorted_years[-1]
    
    latest_ratio = ratios[ratios["year"] == latest_year].iloc[0] if not ratios[ratios["year"] == latest_year].empty else {}
    latest_mcap = mcap[mcap["year"] == latest_year].iloc[0] if not mcap[mcap["year"] == latest_year].empty else {}
    latest_pnl = pnl[pnl["year"] == latest_year].iloc[0] if not pnl[pnl["year"] == latest_year].empty else {}
    latest_cf = cf[cf["year"] == latest_year].iloc[0] if not cf[cf["year"] == latest_year].empty else {}
    latest_bs = bs[bs["year"] == latest_year].iloc[0] if not bs[bs["year"] == latest_year].empty else {}
    ...
    comp_pros_cons = df_pros_cons[df_pros_cons["company_id"] == ticker] if not df_pros_cons.empty else pd.DataFrame()
    ...
    # Custom Styles
    style_header = ParagraphStyle('HeaderStyle', parent=styles['Heading1'], fontName='Helvetica-Bold', fontSize=14, textColor=colors.white, alignment=TA_LEFT)
    style_subheader = ParagraphStyle('SubheaderStyle', fontName='Helvetica', fontSize=9, textColor=colors.white, alignment=TA_RIGHT)
    style_tile_num = ParagraphStyle('TileNum', fontName='Helvetica-Bold', fontSize=12, textColor=HexColor('#002B49'), alignment=TA_CENTER)
    style_tile_lbl = ParagraphStyle('TileLbl', fontName='Helvetica', fontSize=7.5, textColor=HexColor('#4B5563'), alignment=TA_CENTER)
    
    style_bullet_pro = ParagraphStyle('BulletPro', parent=styles['Normal'], fontName='Helvetica', fontSize=8, leading=10, textColor=HexColor('#065F46'))
    style_bullet_con = ParagraphStyle('BulletCon', parent=styles['Normal'], fontName='Helvetica', fontSize=8, leading=10, textColor=HexColor('#991B1B'))
    style_section_title = ParagraphStyle('SectionTitle', fontName='Helvetica-Bold', fontSize=10, textColor=HexColor('#002B49'), spaceAfter=5)
    style_badge = ParagraphStyle('BadgeStyle', fontName='Helvetica-Bold', fontSize=10, textColor=colors.white, alignment=TA_CENTER)
    ...
    header_data = [
        [Paragraph(f"{comp_name} ({ticker})", style_header), Paragraph(f"Sector: {sector_name} | FY: {latest_year}", style_subheader)]
    ]
    ...
    # Retrieve metrics
    rev_val = f"₹{to_float(latest_pnl.get('sales')):.1f} Cr"
    pat_val = f"₹{to_float(latest_pnl.get('net_profit')):.1f} Cr"
    roe_val = f"{to_float(latest_ratio.get('return_on_equity_pct')):.1f}%" if 'return_on_equity_pct' in latest_ratio else "N/A"
    de_val = f"{to_float(latest_ratio.get('debt_to_equity')):.2f}" if 'debt_to_equity' in latest_ratio else "N/A"
    cfo_val = f"₹{to_float(latest_cf.get('operating_activity')):.1f} Cr"
    mcap_val = f"₹{to_float(latest_mcap.get('market_cap_crore')):.1f} Cr" if 'market_cap_crore' in latest_mcap else "N/A"
    
    tile_data = [
        [
            [Paragraph(rev_val, style_tile_num), Paragraph("Revenue (Sales)", style_tile_lbl)],
            [Paragraph(pat_val, style_tile_num), Paragraph("Net Profit (PAT)", style_tile_lbl)],
            [Paragraph(roe_val, style_tile_num), Paragraph("Return on Equity (ROE)", style_tile_lbl)]
        ],
        [
            [Paragraph(de_val, style_tile_num), Paragraph("Debt-to-Equity (D/E)", style_tile_lbl)],
            [Paragraph(cfo_val, style_tile_num), Paragraph("Cash Flow from Ops (CFO)", style_tile_lbl)],
            [Paragraph(mcap_val, style_tile_num), Paragraph("Market Capitalisation", style_tile_lbl)]
        ]
    ]
    ...
    profile_p = Paragraph(about_text, ParagraphStyle('AboutStyle', parent=styles['Normal'], fontName='Helvetica', fontSize=8, leading=11, textColor=HexColor('#374151')))
    ...
    pro_bullet_flowables = [Paragraph("<b>PROS (Key Strengths)</b>", ParagraphStyle('ProTitle', parent=style_section_title, textColor=HexColor('#065F46'), spaceAfter=5))]
    ...
    con_bullet_flowables = [Paragraph("<b>CONS (Risks / Weaknesses)</b>", ParagraphStyle('ConTitle', parent=style_section_title, textColor=HexColor('#991B1B'), spaceAfter=5))]
    ...
    pros_cons_table = Table([[pro_bullet_flowables, con_bullet_flowables]], colWidths=[3.55*inch, 3.55*inch])
    ...
    badge_p = Paragraph(f"CAPITAL ALLOCATION PATTERN: {cap_alloc_label.upper()}", style_badge)
...
def main():
    ...
    for ticker in test_tickers:
        ...
        try:
            ...
            if success:
                print(f"  [OK] Saved tearsheet to {dest_path} ({os.path.getsize(dest_path)} bytes)")
            else:
                print(f"  [SKIPPED] Ticker {ticker} has less than 3 years of financial data.")
