...
def generate_sector_report(sector_id, sector_name, dest_path):
    ...
    df_comp = pd.read_sql(f"SELECT company_id, company_name FROM companies WHERE sector_id = {sector_id}", conn)
    if df_comp.empty:
        conn.close()
        return False
    ...
    for ticker in tickers:
        ...
        # Load latest values
        pnl_row = pd.read_sql(f"SELECT sales, net_profit, operating_profit, depreciation FROM profitandloss WHERE company_id = '{ticker}' AND year = '{latest_yr}'", conn)
        bs_row = pd.read_sql(f"SELECT equity_capital, reserves, borrowings FROM balancesheet WHERE company_id = '{ticker}' AND year = '{latest_yr}'", conn)
        ratio_row = pd.read_sql(f"SELECT return_on_equity_pct, debt_to_equity FROM financial_ratios WHERE company_id = '{ticker}' AND year = '{latest_yr}'", conn)
        mcap_row = pd.read_sql(f"SELECT market_cap_crore FROM market_cap WHERE company_id = '{ticker}' AND year = '{latest_yr}'", conn)
        
        sales = to_float(pnl_row.iloc[0]["sales"]) if not pnl_row.empty else 0.0
        net_profit = to_float(pnl_row.iloc[0]["net_profit"]) if not pnl_row.empty else 0.0
        roe = to_float(ratio_row.iloc[0]["return_on_equity_pct"]) if not ratio_row.empty else 0.0
        de = to_float(ratio_row.iloc[0]["debt_to_equity"]) if not ratio_row.empty else 0.0
        mcap = to_float(mcap_row.iloc[0]["market_cap_crore"]) if not mcap_row.empty else 0.0
    ...
    # Styles
    style_header = ParagraphStyle('SecHeader', parent=styles['Heading1'], fontName='Helvetica-Bold', fontSize=15, textColor=colors.white)
    style_subheader = ParagraphStyle('SecSubheader', fontName='Helvetica', fontSize=9, textColor=colors.white, alignment=TA_RIGHT)
    style_sec_title = ParagraphStyle('SecSectionTitle', fontName='Helvetica-Bold', fontSize=11, textColor=HexColor('#002B49'), spaceAfter=8)
    
    style_cell = ParagraphStyle('Cell', fontName='Helvetica', fontSize=7.5, leading=9, textColor=HexColor('#374151'))
    style_cell_bold = ParagraphStyle('CellBold', fontName='Helvetica-Bold', fontSize=7.5, leading=9, textColor=HexColor('#111827'))
    style_cell_center = ParagraphStyle('CellCenter', fontName='Helvetica', fontSize=7.5, leading=9, textColor=HexColor('#374151'), alignment=TA_CENTER)
    style_cell_right = ParagraphStyle('CellRight', fontName='Helvetica', fontSize=7.5, leading=9, textColor=HexColor('#374151'), alignment=TA_RIGHT)
    
    story = []
    
    # Header Banner
    header_data = [
        [Paragraph(f"Sector Analysis: {sector_name}", style_header), Paragraph(f"Companies: {len(df_sec_data)}", style_subheader)]
    ]
    ...
    medians_table = Table(medians_data, colWidths=[2.43*inch, 2.43*inch, 2.43*inch])
    medians_table.setStyle(TableStyle([
    ...
    comp_table = Table(
        comparison_rows, 
        colWidths=[0.8*inch, 2.0*inch, 0.85*inch, 0.75*inch, 0.7*inch, 0.7*inch, 0.6*inch, 0.9*inch]
    )
...
def main():
    ...
    if success:
        print(f"[OK] Generated test sector report: {dest} ({os.path.getsize(dest)} bytes)")
    else:
