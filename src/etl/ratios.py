...
def main():
    ...
    for key in pnl_idx.index:
        ...
        if (c_id, yr - 3) in pnl_idx.index:
            cagr_rev_3 = calculate_cagr(
                sales, pnl_idx.loc[(c_id, yr - 3)].get("sales"), 3
            )
            cagr_pat_3 = calculate_cagr(
                net_profit, pnl_idx.loc[(c_id, yr - 3)].get("net_profit"), 3
            )
        ...
        if (c_id, yr - 5) in pnl_idx.index:
            cagr_rev_5 = calculate_cagr(
                sales, pnl_idx.loc[(c_id, yr - 5)].get("sales"), 5
            )
            cagr_pat_5 = calculate_cagr(
                net_profit, pnl_idx.loc[(c_id, yr - 5)].get("net_profit"), 5
            )
        ...
        if (c_id, yr - 10) in pnl_idx.index:
            cagr_rev_10 = calculate_cagr(
                sales, pnl_idx.loc[(c_id, yr - 10)].get("sales"), 10
            )
            cagr_pat_10 = calculate_cagr(
                net_profit, pnl_idx.loc[(c_id, yr - 10)].get("net_profit"), 10
            )
            cagr_eps_10 = calculate_cagr(
                eps, pnl_idx.loc[(c_id, yr - 10)].get("eps"), 10
            )
    ...
    print(
        f"[OK] Successfully computed and loaded {len(df_ratios)} rows to financial_ratios table."
    )
