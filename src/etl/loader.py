...
def map_analysis_to_tickers(df_analysis, df_companies):
    ...
    for idx, old_name in enumerate(old_names):
        ...
        for ticker, r_name in zip(real_tickers, real_names):
            ...
            if (
                norm_old in norm_real
                or norm_real in norm_old
                or (len(norm_old) >= 4 and norm_old[:4] in norm_real)
            ):
...
def run_etl():
    ...
    if "documents" in val_dfs:
        val_dfs["documents"] = val_dfs["documents"].rename(
    ...
    sectors_df = pd.DataFrame({"broad_sector": unique_broad})
    ...
    print(
        f"[OK] Loaded sectors ({len(sectors_df)} rows) and companies ({len(companies_df)} rows)"
    )
    ...
    audit_records = [
        {
            "table_name": "sectors",
            "raw_row_count": len(sectors_df),
            "clean_row_count": len(sectors_df),
            "rejected_row_count": 0,
            "status": "SUCCESS",
        },
        {
            "table_name": "companies",
            "raw_row_count": len(companies_df),
            "clean_row_count": len(companies_df),
            "rejected_row_count": 0,
            "status": "SUCCESS",
        },
    ]
    ...
    if violations:
        ...
        conn.close()
        raise ValueError(
            "Database load failed due to Foreign Key integrity violations."
        )
    else:
