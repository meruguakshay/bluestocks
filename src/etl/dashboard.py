...
def main():
    ...
    plt.title("Nifty 100 Company Distribution across Sectors", fontsize=14, weight="bold", pad=15)
    ...
    latest_yr = pd.read_sql_query("SELECT MAX(year) as max_yr FROM profitandloss", conn).iloc[0]["max_yr"]
    ...
    melted = pd.melt(pnl_df, id_vars=["company_id"], value_vars=["sales", "net_profit"], 
                     var_name="Metric", value_name="Amount")
    melted["Metric"] = melted["Metric"].map({"sales": "Sales", "net_profit": "Net Profit"})
    
    sns.barplot(data=melted, x="Amount", y="company_id", hue="Metric", palette="muted")
    plt.title(f"Top 10 Nifty 100 Companies by Sales vs Net Profit (FY {latest_yr})", fontsize=14, weight="bold", pad=15)
    ...
    roe_clean = roe_df[(roe_df["return_on_equity_pct"] > -50) & (roe_df["return_on_equity_pct"] < 100)]
    
    plt.figure()
    sns.histplot(data=roe_clean, x="return_on_equity_pct", kde=True, bins=20, color="teal")
    plt.title(f"Distribution of Return on Equity (ROE %) in FY {latest_yr}", fontsize=14, weight="bold", pad=15)
    plt.xlabel("ROE (%)")
