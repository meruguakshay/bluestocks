...
class FinancialApiHandler(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        if len(parts) == 2 and parts[0] == "api" and parts[1] == "companies":
            ...
            cursor = conn.cursor()
            cursor.execute(
                "SELECT company_id, company_name, market_cap_category FROM companies"
            )
        ...
        # 3. /api/sector/<sector_name>
            ...
            if rows:
                ...
                self._send_response(
                    200, {"sector": sector_name, "companies": constituents}
                )
            else:
        ...
        # 4. /api/ratios/<ticker>
            ...
            if rows:
                ...
                self._send_response(
                    404, {"error": f"No ratios found for company {ticker}"}
                )
            return
