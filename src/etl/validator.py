...
class DataQualityValidator:
    ...
    def run_validation(
        ...
        output_path: str | None = "output/validation_failures.csv",
    ) -> pd.DataFrame:
        """
        ...
        comp_id_col = (
            ...
            else ("id" if "id" in companies.columns else None)
        )
            ...
            if not df.empty and col and col in df.columns:
                ...
                for idx, r in dups.iterrows():
                    ...
                    self.log_failure(
                        ...
                        "DQ-01",
                        "CRITICAL",
                        f"Duplicate primary key {col}: {val}",
                        val,
                    )
        ...
        for tbl_name, df in [
            ...
            if not df.empty and "company_id" in df.columns:
                yr_col = (
                    ...
                    else ("Year" if "Year" in df.columns else None)
                )
                if yr_col:
        ...
        if not companies.empty and comp_id_col:
            valid_company_ids = set(
                companies[comp_id_col].dropna().apply(normalize_ticker).unique()
            )
        ...
        for tbl_name, df in [
            ...
            if (
                ...
                and len(valid_company_ids) > 0
            ):
                orphans = df[
                    ~df["company_id"].apply(normalize_ticker).isin(valid_company_ids)
                ]
        ...
        for tbl_name, df in [
            ...
            if not df.empty:
                col = (
                    ...
                    else ("id" if tbl_name == "companies" else None)
                )
                if col:
                    for idx, r in df.iterrows():
                        ...
                        if (
                            norm_ticker is None
                            or len(norm_ticker) < 2
                            or len(norm_ticker) > 12
                            or not ticker_regex.match(norm_ticker)
                        ):
                            self.log_failure(
                                tbl_name,
                                col,
        ...
        if not docs.empty:
            col = (
                ...
                else ("Annual_Report" if "Annual_Report" in docs.columns else None)
            )
            if col:
