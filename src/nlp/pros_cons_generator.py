...
def main():
    ...
    df_companies = pd.read_sql(
        "SELECT company_id, company_name, roce_percentage, roe_percentage, sector_id FROM companies",
        conn,
    )
    ...
    company_sector_dict = dict(
        zip(df_companies["company_id"], df_companies["broad_sector"])
    )

    # Group data by company for easy time-series lookup
    ratios_by_comp = {
        c: grp.to_dict(orient="records") for c, grp in df_ratios.groupby("company_id")
    }
    pnl_by_comp = {
        c: grp.to_dict(orient="records") for c, grp in df_pnl.groupby("company_id")
    }
    bs_by_comp = {
        c: grp.to_dict(orient="records") for c, grp in df_bs.groupby("company_id")
    }
    cf_by_comp = {
        c: grp.to_dict(orient="records") for c, grp in df_cf.groupby("company_id")
    }
    mcap_by_comp = {
        c: grp.to_dict(orient="records") for c, grp in df_mcap.groupby("company_id")
    }
    ...
    for _, comp_row in df_companies.iterrows():
        ...
        if len(r_hist) >= 5:
            ...
            if all(f > 0.0 for f in fcf_last_5):
                ...
                pos_years = sum(
                    1 for x in r_hist if to_float(x.get("free_cash_flow_cr")) > 0.0
                )
        ...
        if (icr is not None and icr > 10.0) or icr_label == "Debt Free":
            conf = (
                ...
                else min(98.0, 70.0 + (icr - 10.0) * 1.0)
            )
            pros.append(
                {
                    "rule_id": "PRO_7",
        ...
        if len(r_hist) >= 3:
            ...
            if all(f < 0.0 for f in fcf_last_3):
                conf = min(
                    98.0,
                    70.0
                    + sum(1 for x in r_hist if to_float(x.get("free_cash_flow_cr")) < 0)
                    * 5.0,
                )
                cons.append(
                    {
        ...
        if len(r_hist) >= 3:
            opm_vals = [
                to_float(x.get("operating_profit_margin_pct")) for x in r_hist[-3:]
            ]
        ...
        if len(pros) == 0:
            ...
            if latest_fcf is not None and latest_fcf > 0.0:
                ...
                latest_ratio.get("return_on_equity_pct") is not None
                and to_float(latest_ratio.get("return_on_equity_pct")) > 10.0
            ):
                pros.append(
                    {
                        "rule_id": "PRO_10",
    ...
    if len(missing_pros) == 0 and len(missing_cons) == 0:
        print(
            "[OK] Verification passed! Every company has at least 1 pro and at least 1 con."
        )
    else:
        print(
            f"[CRITICAL] Verification failed! Missing Pros: {missing_pros}, Missing Cons: {missing_cons}"
        )
