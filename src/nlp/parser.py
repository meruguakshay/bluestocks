...
def main():
        ...
        for col in target_cols:
            ...
            if parsed:
                ...
                failure_records.append(
                    {"company_id": comp_id, "column_name": col, "raw_value": val}
                )
    ...
    if not df_cross_val.empty:
        df_cross_val.to_csv(
            os.path.join(OUTPUT_DIR, "cross_validation.csv"), index=False
        )
        print(
            f"Saved {len(df_cross_val)} cross-validation rows to output/cross_validation.csv"
        )
        ...
        if diverged_count > 0:
            ...
            print(
                df_cross_val[df_cross_val["diverged_gt_5"] == 1][
                    [
                        "company_id",
                        "metric_type",
                        "parsed_value_pct",
        ...
        print(
            "No conformed 5-year growth values to cross-validate. Saved empty output/cross_validation.csv"
        )
