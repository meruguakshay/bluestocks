...
def main():
    ...
    conformed_keys = sorted(
        [k for k in union_keys if k[0] in valid_companies], key=lambda x: (x[0], x[1])
    )
    ...
    for c_id, yr_str in conformed_keys:
        ...
        operating_profit = (
            pnl_idx.loc[key]["operating_profit"] if key in pnl_idx.index else None
        )
        opm_percentage = (
            pnl_idx.loc[key]["opm_percentage"] if key in pnl_idx.index else None
        )
        other_income = (
            pnl_idx.loc[key]["other_income"] if key in pnl_idx.index else None
        )
        interest = pnl_idx.loc[key]["interest"] if key in pnl_idx.index else None
        depreciation = (
            pnl_idx.loc[key]["depreciation"] if key in pnl_idx.index else None
        )
        ...
        dividend_payout = (
            pnl_idx.loc[key]["dividend_payout"] if key in pnl_idx.index else None
        )

        # Balance Sheet Metrics
        equity_capital = (
            bs_idx.loc[key]["equity_capital"] if key in bs_idx.index else None
        )
        ...
        book_value_per_share = calculate_book_value_per_share(
            equity_capital, reserves, face_val
        )
        ...
        for i in range(5):
            ...
            cfo_list.append(
                cf_idx.loc[offset_key]["operating_activity"]
                if offset_key in cf_idx.index
                else None
            )
            pat_list.append(
                pnl_idx.loc[offset_key]["net_profit"]
                if offset_key in pnl_idx.index
                else None
            )
        ...
        cfo_s, cfi_s, cff_s, alloc_pattern = classify_capital_allocation(
            cfo, cfi, cff, net_profit
        )
    ...
    print(
        f"[OK] Re-created and loaded {len(df_ratios)} rows to financial_ratios table."
    )
