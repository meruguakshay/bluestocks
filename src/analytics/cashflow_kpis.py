...
def main():
    ...
    df_companies = pd.read_sql(
        "SELECT company_id, company_name, sector_id FROM companies", conn
    )
    df_sectors = pd.read_sql("SELECT sector_id, broad_sector FROM sectors", conn)
    df_pnl = pd.read_sql(
        "SELECT company_id, year, sales, net_profit, operating_profit FROM profitandloss ORDER BY company_id, year",
        conn,
    )
    df_bs = pd.read_sql(
        "SELECT company_id, year, borrowings FROM balancesheet ORDER BY company_id, year",
        conn,
    )
    df_cf = pd.read_sql(
        "SELECT company_id, year, operating_activity, investing_activity, financing_activity FROM cashflow ORDER BY company_id, year",
        conn,
    )
    ...
    comp_sector_dict = dict(
        zip(df_companies["company_id"], df_companies["broad_sector"])
    )

    # Group data by company for easy historical lookups
    pnl_by_comp = {
        c: grp.set_index("year").to_dict(orient="index")
        for c, grp in df_pnl.groupby("company_id")
    }
    cf_by_comp = {
        c: grp.set_index("year").to_dict(orient="index")
        for c, grp in df_cf.groupby("company_id")
    }
    bs_by_comp = {
        c: grp.set_index("year").to_dict(orient="index")
        for c, grp in df_bs.groupby("company_id")
    }
    ...
    for _, comp_row in df_companies.iterrows():
        ...
        if (
            len(fcf_history) >= 5
            and fcf_history[0] is not None
            and fcf_history[-1] is not None
        ):
            fcf_cagr_5yr, _ = calculate_cagr(
                fcf_history[-1], fcf_history[0], 4
            )  # 5 years contains 4 intervals
        ...
        _, _, _, alloc_pattern = classify_capital_allocation(
            cfo_val, cfi_val, cff_val, net_profit_val
        )
        ...
        results.append(
            {
                ...
                "sector": sector,
                "cfo_quality_score": (
                    round(cfo_qual_score, 4) if cfo_qual_score is not None else None
                ),
                ...
                "fcf_cagr_5yr": (
                    round(fcf_cagr_5yr, 4) if fcf_cagr_5yr is not None else None
                ),
                "fcf_conversion_pct": (
    ...
    if not df_alerts.empty:
        ...
        print(
            f"Saved {len(df_alerts)} distressed companies to output/distress_alerts.csv"
        )
    else:
        df_empty_alerts = pd.DataFrame(
            columns=["company_id", "cfo_value", "cff_value", "latest_net_profit"]
        )
        df_empty_alerts.to_csv("output/distress_alerts.csv", index=False)
        print(
            "No companies flagged with distress signal. Saved empty output/distress_alerts.csv"
        )
