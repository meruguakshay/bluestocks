...
def main():
    ...
    df_rat_sorted = df_rat_raw.sort_values(by=["company_id", "is_null_fcf", "ends_with_03"], ascending=[True, True, False])
    ...
    df_mcap_5yr = df_mcap_all[df_mcap_all["year"].isin([2020, 2021, 2022, 2023, 2024])].copy()
    # Filter positive PE ratios for a valid median calculation
    df_mcap_5yr_valid = df_mcap_5yr[df_mcap_5yr["pe_ratio"] > 0].copy()
    
    df_median_pe = df_mcap_5yr_valid.groupby("company_id")["pe_ratio"].median().reset_index()
    df_median_pe.columns = ["company_id", "5yr_median_PE"]
    
    # 4. Merge all data sources
    # Start with all 92 companies to ensure we have exactly 92 rows
    df_val = pd.merge(df_comp, df_mcap_2024[[
        "company_id", "market_cap_crore", "pe_ratio", "pb_ratio", "ev_ebitda", "dividend_yield_pct"
    ...
    df_positive_pe = df_val[(df_val["pe_ratio"].notna()) & (df_val["pe_ratio"] > 0)].copy()
